...
export default function Snap({ onChange, onImageUpload, isDisabled }: Props) {
    ...
    return (
        <div className="flex flex-col items-center justify-around gap-3 w-full">
            ...
            <div className={`flex flex-row items-center justify-center gap-3 rounded-xl border border-(--black-2) [&:has(input:focus)]:border-(--brand-6) [&:has(input:focus)]:outline-4 [&:has(input:focus)]:outline-(--brand-3) w-full px-1 py-1 ${isDisabled ? 'bg-gray-200' : 'bg-white '}`}>
                <div className="flex flex-row items-center justify-start gap-4 w-full">
                    ...
                    <button disabled={isDisabled} className="flex rounded-lg px-2 py-2 cursor-pointer hover:bg-(--brand-2)">
